...
$Global:GuiUpdateTimer.Add_Tick({
    ...
    if ($Global:CurrentRobocopyProcess -ne $null -and -not $Global:CurrentRobocopyProcess.HasExited) {
        ...
        if ($exitCode -ge 8) { 
            "  ERROR: Robocopy reported errors for '$srcPathProcessed'. Check Robocopy log for details." | Out-File -FilePath $Global:MainLogFileForSession -Append
            ...
            "  Robocopy completed with warnings/info for '$srcPathProcessed'. Check Robocopy log." | Out-File -FilePath $Global:MainLogFileForSession -Append
...
function StartNextRobocopyProcessOrFinalize {
    if ($Global:CurrentSourceIndexToProcess -lt $FolderPaths.Count) {
        ...
        # Robocopy arguments for verbose output to capture
        # /V (Verbose) /ETA (Estimated Time of Arrival)
        # /TEE is not needed here as we capture stdout.
        # /LOG+ is for the persistent detailed log.
        $robocopyArgsForCapture = @(
            "`"$srcPath`"",
            "`"$destinationForThisSource`"",
            "/E", "/ZB", "/COPY:DATSOU",
            "/R:1", "/W:2", "/MT:8", # Fewer retries for faster GUI feedback if stuck
            "/V", "/ETA", # Verbose output and ETA for GUI
            "/LOG+:`"$($Global:RobocopyPersistentLogFile)`"" # This is the per-source persistent log
        )
        ...
        try {
            # Start Robocopy, redirecting output for GUI
            $Global:CurrentRobocopyProcess = Start-Process -FilePath "robocopy.exe" -ArgumentList $robocopyArgsForCapture -RedirectStandardOutput $Global:CurrentRobocopyTempOutputFile -RedirectStandardError $Global:CurrentRobocopyTempOutputFile -PassThru -NoNewWindow -WindowStyle Hidden
            ...
            $script:overallSuccessInBackup = $false
            # Attempt to proceed to the next item
            $Global:CurrentSourceIndexToProcess++
...
$backupBtn.Add_Click({
    ...
    $Global:BackupSessionTimestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss" # Store session timestamp globally
    ...
    $Global:MainLogFileForSession = Join-Path $mainLogDir "BackupSessionSummary_$($Global:BackupSessionTimestamp).log" # Use global var
    ...
    $script:overallSuccessInBackup = $true # Initialize overall success flag
    $Global:CurrentSourceIndexToProcess = 0 # Reset index
    
    # Define the path for the persistent Robocopy log for the *first* source item. This will be updated in the loop.
    # This is a bit awkward here, better to define it inside the loop for clarity.
    # $firstSrcPath = $FolderPaths[0]
    # $Global:RobocopyPersistentLogFile = Join-Path $mainLogDir "Robocopy_$(Sanitize-FolderName -FullPath $firstSrcPath)_$($Global:BackupSessionTimestamp).log"
    # The above line is problematic if FolderPaths is empty, though we check earlier.
    # It's better to set $Global:RobocopyPersistentLogFile inside StartNextRobocopyProcessOrFinalize for each source.

    StartNextRobocopyProcessOrFinalize # Start the first (or only) backup operation
})
