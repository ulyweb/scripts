...
function StartNextRobocopyProcessOrFinalize {
    if ($Global:CurrentSourceIndexToProcess -lt $FolderPaths.Count) {
        ...
        try {
            # Start Robocopy, redirecting output for GUI. Removed -NoNewWindow as -WindowStyle Hidden implies it.
            $Global:CurrentRobocopyProcess = Start-Process -FilePath "robocopy.exe" -ArgumentList $robocopyArgsForCapture -RedirectStandardOutput $Global:CurrentRobocopyTempOutputFile -RedirectStandardError $Global:CurrentRobocopyTempOutputFile -PassThru -WindowStyle Hidden
