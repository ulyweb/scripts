...
# Relaunch the script with elevated privileges if not already running as administrator
if (-not (Test-IsAdmin)) {
    Write-Host "Script is not running with administrative privileges. Attempting to relaunch as administrator..." -ForegroundColor Yellow
    ...
    try {
        ...
        Write-Error "Failed to relaunch the script with administrative privileges. $_"
    }
    exit
}
