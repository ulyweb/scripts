...
#    return
#}
# Check if Box Drive, Box Tools and Box folder are installed
$boxDrivePath = "C:\Program Files\Box\Box"
$boxToolsPath = "C:\Program Files (x86)\Box\Box Edit"
$boxUserFolder = Join-Path $env:USERPROFILE "Box"

if (-not (Test-Path $boxDrivePath) -or -not (Test-Path $boxToolsPath) -or -not (Test-Path $boxUserFolder)) {
    [System.Windows.Forms.MessageBox]::Show(
        "Box Drive and/or Box Tools are not installed, or the Box folder is missing." + [Environment]::NewLine +
        "Please install Box Drive and Box Tools from the Software Center before running this tool.",
        "Box Software Missing",
...
$userLabel.Text = "Logged in as: $UserName ($UserProfile) || Email Address: $env:EmailAddress "
