...
# Check Box Drive and Tools installation
function Test-BoxEnvironment {
    ...
    if ($missingComponents.Count -gt 0) {
        ...
        $message += "`n`nPlease install them before proceeding."
        ...
        try {
            ...
            Get-ChildItem -Path $src -Recurse -File | ForEach-Object {
                ...
                if ($destPath.Length -ge 260) {
                    Add-Content $logPath "SKIPPED (Path too long): $($_.FullName) => $destPath"
                } else {
