...
$Global:GuiUpdateTimer.Interval = 750 # Check a bit more frequently
...
$Global:GuiUpdateTimer.Add_Tick({
    ...
    if ($Global:CurrentRobocopyProcess -ne $null -and -not $Global:CurrentRobocopyProcess.HasExited) {
        if (Test-Path $Global:CurrentRobocopyTempOutputFile) {
            try {
                $outputLines = Get-Content -Path $Global:CurrentRobocopyTempOutputFile -ErrorAction SilentlyContinue
                if ($null -ne $outputLines) { 
                    $displayLines = $outputLines | Select-Object -Last 10 
                    $Global:RobocopyOutputBox.Text = $displayLines -join [System.Environment]::NewLine
        ...
        if (Test-Path $Global:CurrentRobocopyTempOutputFile) {
             try {
                ...
                if ($null -ne $finalOutputLines) {
                    $Global:RobocopyOutputBox.Text = ($finalOutputLines | Select-Object -Last 20) -join [System.Environment]::NewLine
                    if ($Global:RobocopyOutputBox.TextLength -gt 0) {
                        $Global:RobocopyOutputBox.SelectionStart = $Global:RobocopyOutputBox.TextLength
                        $Global:RobocopyOutputBox.ScrollToCaret()
                    }
