...
$form.Size = New-Object System.Drawing.Size(820, 700) # Increased height for progress bar
...
$totalSizeLabel.Size = New-Object System.Drawing.Size(780, 20) # Wider for longer text
...
$overallProgressBar.Location = New-Object System.Drawing.Point(10, 510) 
$overallProgressBar.Size = New-Object System.Drawing.Size(780, 20)
$overallProgressBar.Visible = $false # Initially hidden
...
function Update-FolderDisplayAndSize {
    ...
    foreach ($path in $FolderPaths) {
        ...
        $form.Update() # Refresh UI to show status message
...
$backupBtn.Add_Click({
    ...
    $overallProgressBar.Maximum = $FolderPaths.Count
    ...
    foreach ($srcPath in $FolderPaths) {
        ...
        $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Starting copy of '$sanitizedSrcFolderName'..."
        "Processing Source ($currentFolderIndex/$($FolderPaths.Count)): $srcPath" | Out-File -FilePath $mainLogFile -Append
        ...
        try {
            ...
            $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Error for '$sanitizedSrcFolderName'. See log."
            $errMsg | Out-File -FilePath $mainLogFile -Append
            $overallSuccess = $false
            # Update progress even if this folder fails, to move to the next
            $overallProgressBar.Value = $currentFolderIndex 
            $form.Update()
            continue 
        }
        
        $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Copying '$sanitizedSrcFolderName' via Robocopy... This may take time."
        $form.Update()
        ...
        $robocopyArgs = @(
            ...
            "/E", "/ZB", "/COPY:DATSOU",
            "/R:2", "/W:5", "/MT:8", # Using 8 threads for copying
            "/NP", "/NFL", "/NDL", "/NJH", "/NJS", # Suppress Robocopy's own progress/summary in console
            "/LOG+:`"$robocopyLogFile`"" # Append to specific log file
        )
        ...
        try {
            ...
            if ($exitCode -ge 8) { 
                ...
                $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Error copying '$sanitizedSrcFolderName'. Check logs."
                $overallSuccess = $false
            } elseif ($exitCode -ge 1 -and $exitCode -lt 8) { 
                "  Robocopy completed with warnings/info for '$srcPath'. Check log: $robocopyLogFile" | Out-File -FilePath $mainLogFile -Append
                 $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Copied '$sanitizedSrcFolderName' with info/warnings."
            } else { # ExitCode 0
                 "  Robocopy completed successfully for '$srcPath'. No files needed copying or no changes." | Out-File -FilePath $mainLogFile -Append
                 $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Verified '$sanitizedSrcFolderName' (no changes)."
            }
        } catch {
            $errMsg = "  CRITICAL ERROR executing Robocopy for '$srcPath'. Error: $($_.Exception.Message)"
            $errMsg | Out-File -FilePath $mainLogFile -Append
            $statusLabel.Text = "Overall: ($currentFolderIndex/$($FolderPaths.Count)) - Robocopy failed for '$sanitizedSrcFolderName'. Check logs."
            $overallSuccess = $false
        }
        
        $overallProgressBar.Value = $currentFolderIndex
        # Status label already updated based on Robocopy outcome, or will be updated at start of next loop / end of process.
        $form.Update()
        "---" | Out-File -FilePath $mainLogFile -Append
    } # End foreach $srcPath
    ...
    $backupBtn.Text = "Start Backup to Box Folder"
    $overallProgressBar.Visible = $false
    # $form.Update() # Done by final message box or status label update

    $finalMessage = "Backup process finished."
    if ($overallSuccess) {
        $finalMessage += " All selected sources processed."
        if ($FolderPaths.Count -gt 0) { # Add if any warnings/info occurred during successful copies
             $finalMessage += " Some operations may have had warnings/info (check logs)."
        }
    ...
    $form.Update() # Ensure final status label text is shown if no message box appeared or after it closes.
})
