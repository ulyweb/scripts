...
function Ensure-ExecutionPolicy {
    ...
    if ($currentPolicy -eq "Restricted") {
        Write-Warning "Your execution policy is set to 'Restricted'. Scripts will not run."
        $response = Read-Host "Do you want to set the policy to 'RemoteSigned'? (Y/N)"
        if ($response -in @("Y", "y", "Yes", "yes")) {
            ...
            Write-Host "Cannot proceed with current execution policy. Exiting." -ForegroundColor Red
            exit
        }
    }
}

# Ensure permissions and execution settings
Ensure-Elevated
